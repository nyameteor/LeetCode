...
class Solution {
    ...
    for (String word : wordDict) {
      if (s.startsWith(word)) {
        ...
        if (!reminderResults.equals(List.of())) {
          List<String> sResults = reminderResults.stream().map(e -> resultAdd(e, word)).collect(Collectors.toList());
          ans.addAll(sResults);
        }
      } else {
        continue;
  ...
  public static void main(String[] args) {
    ...
    System.out
        .println(solution.wordBreak("pineapplepenapple", List.of("apple", "pen", "applepen", "pine", "pineapple")));
